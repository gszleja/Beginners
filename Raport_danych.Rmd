...
```
Baza danych ma prawie w połowie duplikaty które trzeba usunąć
```{r}
hipo_baza <- hipo_baza[!duplicated(hipo_baza$Loan_ID), ]
sum(duplicated(hipo_baza$Loan_ID))
```
aktualnie baza nie ma już zadnych duplikatów 
...
```
W bazie mogą pojawić sie dane które są błędnie zapisane, więc skrypt takie wiersze usuwa 
