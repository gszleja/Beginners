...
kable(srednia_kredytu, format = "html", align = "c",
      col.names = c("Status Zatrudnienia", "Średnia Kwota Pożyczki (PLN)")) %>%
...
```{r}
summary(hipo_baza[, c("ApplicantIncome", "CoapplicantIncome", "LoanAmount")])
...
kable(wyniki_freq, format = "html", align = "c",
      col.names = c("Wartość","Freq", "%","% Cum.")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "bordered"),
                full_width = FALSE, position = "center") %>%
  column_spec(1:3, bold = TRUE, background = "#f2f2f2") %>%
  row_spec(0, bold = TRUE, color = "white", background = "#FF0000") %>%  # Zmiana koloru nagłówka
  add_header_above(c("Podstawowe dane statystyczne" = 4), color = "white", background = "#FF0000")
...
# Upewnij się, że zmienne są kategoryczne
...
tabela_krzyzowa <- ctable(hipo_baza$Credit_History, hipo_baza$LoanAmount_cat, prop = "r", round.digits = 2)
...
print(podsumowanie, method = "render")
